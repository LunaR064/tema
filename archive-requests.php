...
<div class="requests">
	<nav class="requests_main_nav">
		<h1><?php _d('Requests List'); ?> <span><?php echo total_requests(); ?></span></h1>
	...
	<div id="requests" class="content">
		<div class="tabox current" id="listrequests">
			<div class="items">
            <?php if (have_posts()) { while (have_posts()) { the_post(); $meta = dt_get_meta('_dbmv_requests_post'); ?>
                    <article id="post-<?php the_ID(); ?>" class="item animation-1">
    					<div class="box">
    						<div data-id="<?php the_ID(); ?>" class="poster post-request"><img src="<?php echo 'https://image.tmdb.org/t/p/w185'. $meta['poster']; ?>" alt="<?php the_title(); ?>"></div>
    						<h3><?php the_title(); ?></h3>
    					</div>
    				</article>
                <?php } } ?>
		...
		<div class="tabox" id="addrequests">
